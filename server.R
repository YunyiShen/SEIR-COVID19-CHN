...
function(input, output, session) {
  ...
  output$plot0 = renderPlotly({
    ...
    out.df2=rename(out.df, c(S="疑似", E="暴露", I0="无症状感染", I1="轻症", I2="重症", I3="危重症", R="康复", D="死亡"))
    ...
    Comb.df2=rename(Comb.df, c(S="疑似", E="暴露", I="感染", R="康复", D="死亡"))
    ...
    p=layout(p,xaxis=list(title="首个病例后天数 (天)"),yaxis=list(title=paste("例每",formatC(N,big.mark=",",format="f",digits=0),"人"),type=input$yscale),
             annotations=list(text=HTML(paste("R", tags$sub(0),'=',formatC(Ro,digits=2)," <br>r =", formatC(r,digits=2)," 每天 <br>T",tags$sub(2)," = ",formatC(DoublingTime,digits=2)," 天")),
    ...
    if(input$AllowSeason=="Yes"){
      ...
      p=layout(p,annotations=list(text=HTML(paste("季节性 R", tags$sub(0), ": <br>初始 R", tags$sub(0),'=',formatC(Ro.Season$Ro.now,digits=2),"<br>峰值 R", tags$sub(0),'=',formatC(Ro.Season$Ro.max,digits=2),"@day",formatC(tpeak,format = "f",digits=0),"<br>最小 R", tags$sub(0),'=',formatC(Ro.Season$Ro.min,digits=2),"@day",formatC(tmin,format = "f",digits=0))),
  ...
  output$plotInt = renderPlotly({
    ...
    out.df$Intervention="基线" # add intervention column
    outInt.df$Intervention="采取措施"
    ...
    p=layout(p,xaxis=list(title="首个病例后天数 (天)"),yaxis=list(title=paste("例每", formatC(N,big.mark=",",format="f",digits=0),"人"),type=input$yscaleInt),
             annotations=list(text=HTML(paste("基线: <br>R", tags$sub(0),'=',format(Ro,nsmall=1)," <br>r =", format(r,digits=2)," 每天 <br>T",tags$sub(2)," = ",format(DoublingTime,digits=1)," 天 <br><br>措施后: <br>R", tags$sub(0),'=',RoInt,"<br>r =", format(rInt,digits=2)," 每天 <br>T",tags$sub(2)," = ",format(DoublingTimeInt,digits=1)," 天")),
    ...
    if(input$AllowSeason=="Yes"){
      ...
      p=layout(p,annotations=list(text=HTML(paste0("季节性 R", tags$sub(0), ": <br>初始 R", tags$sub(0),'=',formatC(Ro.Season$Ro.now,digits=2),"/",formatC(RoInt.Season$Ro.now,digits=2),"<br>峰值 R", tags$sub(0),'=',formatC(Ro.Season$Ro.max,digits=2),"/",formatC(RoInt.Season$Ro.max,digits=2),"<br>  @day ",formatC(tpeak,format = "f",digits=0),"<br>最小 R", tags$sub(0),'=',formatC(Ro.Season$Ro.min,digits=2),"/",formatC(RoInt.Season$Ro.min,digits=2),"<br>  @day ",formatC(tmin,format = "f",digits=0))),
  ...
  output$plotCap = renderPlotly({
    ...
    if(input$VarShowCap=="I3mv"){
      ...
      capData=data.frame("time"=seq(0, Tmax, length.out = 1e3),"value"=rep(1,1e3)*capParams["ConvVentCap"]*(N/1000), "Intervention"="常规 \n 呼吸机容量")
      combData=rbind(outAll.sub,capData)
      capData=data.frame("time"=seq(0, Tmax, length.out = 1e3),"value"=rep(1,1e3)*capParams["ContVentCap"]*(N/1000), "Intervention"="偶发\n 呼吸机容量")
      combData=rbind(combData,capData)
      capData=data.frame("time"=seq(0, Tmax, length.out = 1e3),"value"=rep(1,1e3)*capParams["CrisisVentCap"]*(N/1000), "Intervention"="危重 \n 呼吸机容量")
      combData=rbind(combData,capData)
      ...
      capData=data.frame("time"=seq(0, Tmax, length.out = 1e3),"value"=rep(1,1e3)*capParams["AvailICUBeds"]*(N/1000), "Intervention"="空闲ICU床位")
      combData=rbind(outAll.sub,capData)
      ...
      capData=data.frame("time"=seq(0, Tmax, length.out = 1e3),"value"=rep(1,1e3)*capParams["AvailHospBeds"]*(N/1000), "Intervention"="空闲住院床位")
      combData=rbind(outAll.sub,capData)
    ...
    p=layout(p,xaxis=list(title="首个病例后天数 (天)"),yaxis=list(title=paste("例每",formatC(N,big.mark=",",format="f",digits=0),"人"),type=input$yscaleCap), 
             annotations=list(text=HTML(paste("基线: <br>R", tags$sub(0),'=',format(Ro,nsmall=1)," <br>r =", format(r,digits=2)," 每天 <br>T",tags$sub(2)," = ",format(DoublingTime,digits=1)," 天 <br><br>措施后: <br>R", tags$sub(0),'=',RoInt,"<br>r =", format(rInt,digits=2)," 每天 <br>T",tags$sub(2)," = ",format(DoublingTimeInt,digits=1), " 天")),
  ...
  output$HospBedper <- renderUI({
    numericInput("HospBedper","总数 （每千人）",value = signif(hdata$HospBedper,digits=3), min = 0, step = 0.1)
  })
  output$HospBedOcc <- renderUI({
    numericInput("HospBedOcc","占用 (%)",value = signif(hdata$HospBedOcc,digits=3)*100, min = 0, max = 100, step = 0.1)
  })
  output$ICUBedper <- renderUI({
    numericInput("ICUBedper","总数 （每千人）",value = signif(hdata$ICUBedper,digits=3), min = 0, step = 0.01)
  })
  output$ICUBedOcc <- renderUI({
    numericInput("ICUBedOcc","占用 (%)",value = signif(hdata$ICUBedOcc,digits=3)*100, min = 0, max = 100, step = 1)
  })
  output$IncFluOcc <- renderUI({
    numericInput("IncFluOcc","流感所致占用率上升 (%)",value = signif(hdata$IncFluOcc,digits=3)*100, min = 0, max = 100, step = 1)
  })
  output$ConvVentCap <- renderUI({
    numericInput("ConvMVCap","常规",value = signif(hdata$ConvMVCap,digits=3), min = 0, step = 0.01)
  })
  output$ContVentCap <- renderUI({
    numericInput("ContMVCap","偶发",value = signif(hdata$ContMVCap,digits=3), min = 0, step = 0.01)
  })
  output$CrisisVentCap <- renderUI({
    numericInput("CrisisMVCap","危重",value = signif(hdata$CrisisMVCap,digits=3), min = 0, step = 0.01)
  })
